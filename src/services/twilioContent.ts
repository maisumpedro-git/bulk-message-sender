...
export async function listContentTemplates(
  page: number,
  pageSize = 50,
): Promise<{ items: ContentTemplateResult[]; hasMore: boolean; page: number }> {
  ...
  try {
    ...
    const approved = rawItems.filter((r) => {
      const status =
        ...
        r?.approval_requests?.approval_request_status ||
        r?.approval_requests?.[0]?.status;
      return status === 'approved';
    });
    const items: ContentTemplateResult[] = approved
      .map(mapTwilioContentToTemplate)
