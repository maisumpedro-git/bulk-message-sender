...
const mappingsSchema = z.array(z.object({ variable: z.string().min(1), columnKey: z.string().min(1) }));
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!sessionId || typeof sessionId !== 'string') return NextResponse.json({ error: 'sessionId obrigatório' }, { status: 400 });
    if (!phoneColumn || typeof phoneColumn !== 'string') return NextResponse.json({ error: 'phoneColumn obrigatório' }, { status: 400 });
    if (!file || !(file instanceof File)) return NextResponse.json({ error: 'Arquivo CSV ausente' }, { status: 400 });

    let variableMappings: Array<{ variable: string; columnKey: string }> = [];
    if (variableMappingsRaw && typeof variableMappingsRaw === 'string' && variableMappingsRaw !== '[]') {
      const parsed = JSON.parse(variableMappingsRaw);
      const validated = mappingsSchema.safeParse(parsed);
      if (!validated.success) return NextResponse.json({ error: 'Mapeamentos inválidos', details: validated.error.flatten() }, { status: 400 });
    ...
    if (!records[0].hasOwnProperty(phoneColumn)) return NextResponse.json({ error: 'Coluna de telefone inexistente' }, { status: 400 });
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      const contactsData = records.map(r => ({ phone: (r[phoneColumn] || '').trim(), rawData: r, contactListId: list.id }));
      ...
      if (variableMappings.length) {
        ...
        await (tx as any).sessionVariableMapping.createMany({ data: variableMappings.map(m => ({ sessionId, variable: m.variable, columnKey: m.columnKey })) });
      }
